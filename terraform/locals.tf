...
resource "null_resource" "coredns_run_script" {
  ...
  #   }
  depends_on = [aws_eks_addon.coredns,aws_eks_addon.aws_ebs_csi_driver]

}

## This is the resource to run the Local Script.
resource "null_resource" "redis_helm_cluster_run_script" {
  provisioner "local-exec" {
    command    = "bash ${path.module}/../charts/run_cluster.sh -a eks-cluster-1a -b eks-cluster-1b"
    on_failure = fail
  }
  # triggers = {
  #     always_run = "${timestamp()}"  # Use a timestamp or another always-changing value
 ...
 depends_on = [null_resource.coredns_run_script]
}